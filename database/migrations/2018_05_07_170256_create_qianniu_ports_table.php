...
class CreateQianNiuPortsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('qianniu_ports', function (Blueprint $table) {
            ...
            //线下启动
            $table->tinyInteger('start_charge_type')->unsigned()->nullable()->comment('启动类型 刷卡/1 投币/2');
            $table->integer('balance')->unsigned()->default(0)->comment('充电金额（单位:分）');
            $table->integer('duration')->unsigned()->default(0)->comment('充电时长（单位:分钟）');
            $table->string('card_num',20)->default("")->commnet('卡号');
    ...
    {
        Schema::dropIfExists('qianniu_ports');
    }
}
