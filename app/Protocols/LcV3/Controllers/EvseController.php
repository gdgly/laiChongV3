<?php
namespace Wormhole\Protocols\LcV3\Controllers;
/**
 * Created by PhpStorm.
 * User: sc
 * Date: 2017-05-12
...
use Wormhole\Protocols\LcV3\Models\ModifyInfoLog;
use Wormhole\Protocols\LcV3\Models\CardInfo;
use Wormhole\Protocols\LcV3\Protocol;
...
use Wormhole\Protocols\LcV3\Jobs\AutoStopCharge;

//日结上报,如果monitor未成功接收到数据,继续队列发送
use Wormhole\Protocols\LcV3\Jobs\Report;

//心跳队列
use Wormhole\Protocols\LcV3\Jobs\CheckHeartbeat;
...
//线下启动响应
use Wormhole\Protocols\LcV3\Protocol\Server\OfflineStart;
...
class EvseController extends Controller
{
    ...
    public function checkHeartbeat($id){
        ...
        if((time() - strtotime($hearbeat->last_update_status_time)) < 600){ //$hearbeat->heartbeat_cycle * 60
            ...
            $monitorCodes = MonitorServer::offline($hearbeat->code);
    ...
    public function checkSetParameter($parameterName, $parameter, $code){
        ...
        if(is_array($parameter)){
            ...
            if($requestResult->$parameterName == 1){
                Log::debug(__NAMESPACE__ .  "/" . __CLASS__."/" . __FUNCTION__ . "@" . __LINE__ . " 设置参数成功 parameter:".json_encode($parameter));
    ...
    public function signReport($code, $num, $device_identification, $heabeat_cycle, $worker_id, $version){
        ...
        //枪口列表
        $portArr = [];
        Log::debug(__CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 签到,处理数据start code:$code");
        //桩是否存在
        $evse = Evse::where("code",$code)->first(); //firstOrFail
        Log::debug(__CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 签到 ".json_encode($evse));
        //设备类型
        $evse_type = 1;
        //找不到添加桩和枪
        if(empty($evse)){
            ...
            $monitorCodes = MonitorServer::deviceOnline($code, $num, $portArr, $evse_type, $version);
...
//                 }
        ...
        //调用monitor接口
        $monitorCodes = MonitorServer::deviceOnline($code, $num, array(), $evse_type, $version);
    ...
    public function signResponse($code, $worker_id){
        ...
        $fiel_data = " 签到,响应充电桩 结果 $sendResult, 帧frame: ".bin2hex($frame).'时间:'.Carbon::now();
        $redis_data = json_encode(['frame_name'=>'签到,应答桩', 'protocol_name'=>env("PROTOCOL_NAME"), 'version'=>env("VERSION"), 'parameter'=>array('code'=>$code, 'serialNumber'=>$serialNumber, 'sendResult'=>$sendResult), 'frame'=>bin2hex($frame), 'date'=>strval(Carbon::now())]).'+';
    ...
    public function heartBeatReport($dataArray, $evse_num, $clientID){
        ...
        foreach ($dataArray as $k=>$data) {
            ...
            $status[$k]['self_evse_no'] = $code;
            $status[$k]['protocol_name'] = $evse['protocol_name'];
            //$status[$k]['gun_num'] = $i;
            ...
            for ($i=0; $i < $data['num']; $i++) {
                ...
                $status[$k]['gun_status_list'][$i]['gun_num'] = $i;
                $status[$k]['gun_status_list'][$i]['work_status'] = $statu[$data['status']['worke_state'][$i]];
...
//                    }
                    ...
                    //如果是在充电中,则把剩余时间存储到redis中
                    if($data['status']['worke_state'][$i] == 1){
                        $prefix = env('FRAME_PREFIX'); //前缀
                        $term_validity = env('FRAME_TERM_VALIDITY');//有效期
                        //追加到redis中
                        $key = $prefix.$code.'_'.$i.'_'.'left_time';
                        Redis::set($key, $data['left_time'][$i]);
                        Redis::expire($key, $term_validity);
                    }

                    //判断是否故障
                    if($data['status']['fuses'][$i] == 1 || $data['status']['overcurrent'][$i] == 1){
                        //心跳工作状态故障
                        $status[$k]['gun_status_list'][$i]['work_status'] = 3;//故障

                        Log::debug(__CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 心跳,枪口故障 code1:$code,  code2:".$data['code']);

                        //如果故障,判断是否已经给过monitor
                        $fault = Redis::get($code.'-'.$i.':fault');
                        Log::debug(__CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 心跳,枪口故障1 fault:$fault");
                        if($fault != 1){
                            Redis::set($code.'-'.$i.':fault',1);
                            $fault = Redis::get($code.'-'.$i.':fault');
                            Log::debug(__CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 心跳,枪口故障2 fault:$fault");
                            MonitorServer::add_device_error_log($code, $i, 1);//1为故障
                        }
                        ...
                        if($fault != 2){
                            ...
                            MonitorServer::add_device_error_log($code, $i, 2); //2为正常
                        }
                    ...
                    $port->left_time = $data['left_time'][$i];//$status[$k]['remaining_time'][$i] = $data['left_time'][$i];
                    $port->current = $data['current'][$i];//$status[$k]['current'][$i] = $data['current'][$i];
                    $port_res = $port->save();
                    //json_encode(array("$code.'_left_time'"=>$data['left_time'][$i]));
                    if(empty($port_res)){
           ...
           // Log::debug(__CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 心跳数组: ".json_encode($status));die;
    ...
    public function hearbeatAnswer($dataArray, $evse_num, $clientID){
        ...
        for($i=0;$i<$evse_num;$i++){
            ...
            $frames = $frames.$frame;
            $evses['code'][$i] = $dataArray[$i]['code'];
        }
        $sendResult  = EventsApi::sendMsg($clientID,$frames);
        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " frames: ".bin2hex($frames)." 多个心跳应答桩sendResult:$sendResult " . Carbon::now());

        $fiel_data = " 心跳,应答充电桩 结果 $sendResult, 帧frame: ".bin2hex($frames).'时间:'.Carbon::now();
        $redis_data = json_encode(['frame_name'=>'心跳,应答充电桩', 'protocol_name'=>env("PROTOCOL_NAME"), 'version'=>env("VERSION"), 'parameter'=>$evses, 'frame'=>bin2hex($frame), 'date'=>strval(Carbon::now())]).'+';


        Common::record_log($evses['code'][0], $fiel_data, $redis_data);
    ...
    //自动停止上报
    public function autoStopReport($code, $order_number, $startup_type, $left_time, $stop_reason){
        ...
        $fiel_data = " 自动停止,应答充电桩 结果 $sendResult, 帧frame: ".bin2hex($frame).'时间:'.Carbon::now();
        $redis_data = json_encode(['frame_name'=>'自动停止,应答充电桩', 'protocol_name'=>env("PROTOCOL_NAME"), 'version'=>env("VERSION"), 'parameter'=>array('order_number'=>$order_number, 'result'=>1), 'frame'=>bin2hex($frame), 'date'=>strval(Carbon::now())]).'+';
        Common::record_log($code, $fiel_data, $redis_data);
        ...
        //实际充电时间
        $already_charge_time = $chargeArgs - $left_time;
        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 自动停止,实际充电时间 chargeArgs:$chargeArgs, 
         left_time:$left_time, already_charge_time:$already_charge_time" . Carbon::now());
        //停止成功,调用monitor接口
        $job = (new AutoStopCharge($monitorOrderId, $left_time, $already_charge_time))
            ->onQueue(env("APP_KEY"));
        dispatch($job);
    ...
    public function turnoverReport($code, $meter_number, $date_info, $electricity, $total_electricity, $coins_number, $card_amount, $card_time){
        ...
        if(!empty($turnoverData)){
            ...
            $sameDayCoinsNumber = $coins_number - $coinNumber; //前天的投币数量

        }else{
            $sameDayCoinsNumber = $coins_number;
        }
        ...
        if(empty($turnover)){
            ...
            $result = Turnover::create([
                ...
                'coin_number'=>$coins_number,//$sameDayCoinsNumber,
            ...
            $turnover->coin_number = $coins_number;//$sameDayCoinsNumber = ($coins_number - $coinNumber);
        ...
        $electricity_num = round($electricity_num, 2); //保留两位小数
        //获取workeId
        $evse = Evse::where("code",$code)->first(); //firstOrFail
        $workeId = $evse->worker_id;
        ...
        $fiel_data = " 日结,应答充电桩 结果 $sendResult, 帧frame: ".bin2hex($frame).'时间:'.Carbon::now();
        $redis_data = json_encode(['frame_name'=>'日结,应答充电桩', 'protocol_name'=>env("PROTOCOL_NAME"), 'version'=>env("VERSION"), 'parameter'=>array('time'=>$time, 'date_info'=>$date_info, 'result'=>1), 'frame'=>bin2hex($frame), 'date'=>strval(Carbon::now())]).'+';
        Common::record_log($code, $fiel_data, $redis_data);
        ...
        //停止成功,调用monitor接口
        $job = (new Report($code, $coins_number, $sameDayCoinsNumber, $card_amount, $electricity_num, $total_electricity, $date))
            ->onQueue(env("APP_KEY"));
        dispatch($job);
    ...
    //卡信息上报
    public function cardInfo($code, $card_num, $card_type, $balance, $user_password, $start_card_date, $effective_month){

        //TODO 卡号是否有重复
        $card_info = cardInfo::where("card_num",$card_num)->first();

        //判断卡的类型(160为充值卡,表示余额,161为包月卡,剩余时长)
        if($card_type == 160){
            $balance = $balance / 100;
        }

        //判断开卡时间是否为0
        if($start_card_date == 0){
            $start_card_date = null;
        }
        //找不到卡则创建,否则更新
        if(empty($card_info)){
            $result = cardInfo::create([
                'card_num'=>$card_num,
                'card_type'=>$card_type,
                'balance'=>$balance,
                'user_password'=>$user_password,
                'start_card_date'=>$start_card_date,
            ...
            $card_info->user_password = $user_password;
            $card_info->start_card_date = $start_card_date;
            $card_info->effective_month = $effective_month;
            $card_info->save();
        }

        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 卡片信息 end " . date('Y-m-d H:i:s', time()));
    ...
    //线下启动
    public function offlineStart($code, $odd_num, $channel_num, $start_type, $balance, $duration, $card_num, $client_id){
        $monitor_order_id = '';
        $condition = [
            ['code', '=', $code],
        ...
        $port = Port::where($condition)->first();//firstOrFail
        if(empty($port)){
            Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 线下启动充电,未找到枪 code:$code, port_number:$channel_num ");
            return false;
        }
        ...
        //如果在充电中则取出订单号给monitor
        if($port->work_status != 0 || $port->work_status != 3){
            $monitor_order_id = $port->monitor_order_id;
            $start_charge_type = $port->start_charge_type;
            Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 线下启动充电,当前枪口在充电中 code:$code, channel_num:$channel_num, monitor_order_id:$monitor_order_id");

            //如果启动类型是时间,那么把时间相加
            if($port->charge_type == 1){
                $duration_total = $port->charge_args + $duration; //充电时长,增加时间
                $left_time = $port->left_time + $duration; //剩余时长添加时间
                $balance_total = $port->balance + $balance;
                Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 线下启动充电,当前枪口在充电中 时间: ".$port->charge_args." 金额: ".$port->balance);
            }

        }

        $card_start = Config::get('evseStatus.card_start'); //刷卡
        $coin_start = Config::get('evseStatus.coin_start'); //投币


        //转换启动类型,桩上报(1刷卡,2投币) monitor(5投币,6刷卡)
        $monitor_start_type = $start_type == $card_start ? 6 : ($start_type == $coin_start ? 5 : $start_type);
        //线下启动调用monitor
        $result = MonitorServer::offline_start_charge($code, $channel_num, $balance, $duration, $monitor_start_type, $monitor_order_id);
        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 线下启动充电,调用monitor start_type:$start_type, 结果:".json_encode($result));

        if(empty($monitor_order_id)){
            $monitor_order_id = $result['order_id'];
        }

        //更新枪表数据
        $port->start_charge_type = is_numeric($start_charge_type) ? $start_charge_type : $start_type; //$start_type;
        $port->balance = $balance_total;
        $port->charge_args = $duration_total == 0 ? $port->charge_args : $duration_total;
        $port->charge_type = 1;
        $port->card_num = $card_num;
        $port->monitor_order_id = $monitor_order_id;
        $port->order_id = $odd_num;
        $port->work_status = 2; //状态,启动中
        $port->left_time = $left_time == 0 ? $port->left_time : $left_time;
        ...
        //从redis中取出流水号,如果没有设置为1
        $serialNumber = $this->serial_num($code);
        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 线下启动应答,流水号serialNumber：$serialNumber ");

        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 线下启动应答桩start " . Carbon::now());
        $offlineStart = new OfflineStart();
        $offlineStart->code($code);
        $offlineStart->serial_number(intval($serialNumber));
        $offlineStart->channel_num(intval($channel_num));
        $offlineStart->odd_num(intval($odd_num));
        $offlineStart->result(1);
        $frame = strval($offlineStart);
        $sendResult  = EventsApi::sendMsg($client_id,$frame);//self::$client_id
        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 线下启动响应充电桩结果,sendResult:$sendResult " . Carbon::now());

        $fiel_data = " 线下启动,响应充电桩 结果 $sendResult, 帧frame: ".bin2hex($frame).'时间:'.Carbon::now();
        $redis_data = json_encode(['frame_name'=>'线下启动,应答桩', 'protocol_name'=>env("PROTOCOL_NAME"), 'version'=>env("VERSION"), 'parameter'=>array('code'=>$code, 'serialNumber'=>$serialNumber, 'sendResult'=>$sendResult), 'frame'=>bin2hex($frame), 'date'=>strval(Carbon::now())]).'+';
        Common::record_log($code, $fiel_data, $redis_data);
    }
    ...
    public function startChargeSend($monitorOrderId, $monitorCode, $chargeType, $chargeArgs, $orderId){
        ...
        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 启动充电,monitorOrderId:$monitorOrderId,
         monitorCode:$monitorCode, chargeType:$chargeType, chargeArgs:$chargeArgs, orderId:$orderId");
        //更新枪数据
        ...
        //启动类型
        $onlineStartup = Config::get('evseStatus.online_startup');
        ...
        $port->is_response = 0;
        $port->operation_time = 0;
        $port->start_charge_type = $onlineStartup; //线上启动
        $res = $port->save();
        //如果没有更新成功
        if(empty($res)){
        ...
        $redis_data = json_encode(['frame_name'=>'启动充电下发', 'protocol_name'=>env("PROTOCOL_NAME"), 'version'=>env("VERSION"), 'parameter'=>array('monitorOrderId'=>$monitorOrderId, 'monitorCode'=>$monitorCode, 'chargeType'=>$chargeType, 'chargeArgs'=>$chargeArgs, 'orderId'=>$orderId), 'frame'=>bin2hex($frame), 'date'=>strval(Carbon::now())]).'+';
    ...
    public function renewSend($monitorOrderId, $monitorCode, $chargeType, $chargeArgs, $orderId){
        ...
        $redis_data = json_encode(['frame_name'=>'续费下发', 'protocol_name'=>env("PROTOCOL_NAME"), 'version'=>env("VERSION"), 'parameter'=>array('monitorOrderId'=>$monitorOrderId, 'monitorCode'=>$monitorCode, 'chargeType'=>$chargeType, 'chargeArgs'=>$chargeArgs, 'orderId'=>$orderId), 'frame'=>bin2hex($frame), 'date'=>strval(Carbon::now())]).'+';
    ...
    public function stopChargeSend($monitorOrderId){
        ...
        $redis_data = json_encode(['frame_name'=>'停止充电', 'protocol_name'=>env("PROTOCOL_NAME"), 'version'=>env("VERSION"), 'parameter'=>array('monitorOrderId'=>$monitorOrderId, 'code'=>$code, 'orderId'=>$orderId, 'channelNumber'=>$channelNumber, 'serialNumber'=>$serialNumber), 'frame'=>bin2hex($frame), 'date'=>strval(Carbon::now())]).'+';
    ...
    public function stopChargeResponse($code, $order_number, $result, $left_time, $stop_time){
        ...
        $stopReason = Config::get('evseStatus.stop_reason'); //停止原因,手动点击app停止
        ...
        if($result){
            ...
            $res = ChargeRecords::create([
                ...
                'start_time'=>$port->start_time,
                'end_time'=>Carbon::now(),
                'stop_reason'=>$stopReason, //停止原因添加?
                'startup_type'=>$port->start_charge_type,
                'charged_power'=>$port->charge_args,
                'left_time'=>$left_time,
                'charge_records_time'=>Carbon::now(),
...
//            dispatch($job);
    ...
    public function Report($code, $coins_number, $same_day_coins_number, $card_amount, $electricity_num, $total_electricity, $date){

        //$res = 1;
        $res = MonitorServer::evse_report_day_data($code, $coins_number, $same_day_coins_number, $card_amount, $electricity_num, $total_electricity, $date);
        Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 日结,调用monitor $res: ".json_encode($res) .' '. Carbon::now());
        //如果monitor接受失败,继续发送
        if(empty($res)){
            Log::debug(__NAMESPACE__ . "/" . __CLASS__ . "/" . __FUNCTION__ . "@" . __LINE__ . " 日结失败,继续调用 " . Carbon::now());
            $job = (new Report($code, $coins_number, $same_day_coins_number, $card_amount, $electricity_num, $total_electricity, $date))
                ->onQueue(env("APP_KEY"))
                ->delay(Carbon::now()->addSeconds(1800)); //如果monitor接受日结失败,则半小时后继续发
            dispatch($job);
        }
